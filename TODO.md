...
- [ ] ACTIVE — small follow-up PR requested by Daril: /rules/ask should default confirmedContext to the app's single supported scope (LIHTC / boston_cambridge_quincy_ma_nh_hmfa / 2026) when the caller sends none (session-owns-scope; the Understand UI already sends it explicitly — this is belt-and-suspenders), with a test. Then continue with C7 server half.
