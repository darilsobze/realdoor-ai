...
- [ ] ACTIVE — C7 Safety Test Panel (taken over from Emmanuel; branch `daril/c7-safety-panel`, route in web/src/ui). One page, five LIVE demonstrations against existing endpoints, each with a "Run this check" button, the actual request/response shown, and a plain-language explanation of what it proves:
  ...
  Panel copy: calm info styling (proofs, not errors); no scores, no eligibility language. Coordinate the filename fixture string with Hoan's C7 tests.
