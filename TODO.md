...
- [ ] Polish request from Daril (with C7 panel work): packet "Confirmed values" section shows raw field names/values + document UUIDs — use plain-language labels + formatted values + display names for print quality (buildPacketSections could accept a label/format map so the engine stays UI-free).
