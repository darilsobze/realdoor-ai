...
export type ConfirmedHouseholdSize = {
  value: number | null;
  confirmed: boolean;
};
...
export function annualize(
  ...
  return computed(
    ...
    [
      {
        ...
        value: amount.value,
        unit: `USD/${frequency}`,
        source_field: amount.sourceField,
      },
      {
    ...
    `${roundToCents(amount.value).toFixed(2)} USD/${frequency} × ${multiplier} = ${result.toFixed(2)} USD/year`,
    result,
    INCOME_RULE_ID,
  );
}
...
export function sumIncomeSources(
  ...
  const completed = sources as ComputedCalculation[];
  const values = completed.map((source) => source.result_value);
...
export function compareToThreshold(
  ...
  if (!householdSize.confirmed || householdSize.value === null) {
    return blocked(
      "threshold_comparison",
      householdSize.confirmed ? "household_size_missing" : "input_not_confirmed",
      "Cannot select a published threshold until household size is confirmed.",
      ["household_size"],
    );
  }
  const threshold = rule.thresholds?.find((row) => row.household_size === householdSize.value);
  if (!threshold) {
    return blocked(
      "threshold_comparison",
      "household_size_missing",
      "No published threshold exists for the confirmed household size.",
      ["household_size"],
    );
  }
  ...
  return computed(
    ...
    [
      {
        name: "annual_income",
        value: annualIncome.result_value,
        unit: "USD/year",
        source_field: null,
      },
      {
      ...
      },
      {
        ...
        value: threshold.annual_income_limit_usd,
        unit: "USD/year",
        source_field: null,
      },
    ],
